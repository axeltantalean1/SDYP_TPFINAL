...
void gravitacionCPU(int id){
    ...
    if(idW==0){
	for(int i = 0; i < 3; i++) {
				printf("Fuerza en cuerpo %d hilo id: %d ANTES - fx: %f, fy: %f, fz: %f\n",i,id, 
				fuerza_totalX[i], fuerza_totalY[i], fuerza_totalZ[i]);		
		}
	}
    ...
    if(idW==0){
	for(int i = 0; i < 3; i++) {
				printf("Fuerza en cuerpo %d hilo id: %d DESP - fx: %f, fy: %f, fz: %f\n",i,id,
				fuerza_totalX[i], fuerza_totalY[i], fuerza_totalZ[i]);		
		}
